...
class BigqueryInterface:
    ...
    def create_dataset(self):
        try:
            dataset = bigquery.Dataset(self.get_dataset_ref())
            dataset.location = "EU"
    ...
    def query_to_bq(self,
                    ...
                    time_partition_by: dict = None) -> None or float:
        """
        ...
        :param returns_cost: if True, returns the cost incurred by the query
        ...
        :return: the cost incurred by the query if requested; else None
        """
        ...
        query_job.result()
        if returns_cost:
            return self._get_cost_from_processed_bytes(total_bytes_processed=query_job.total_bytes_processed)

    def run_ddl(self, ddl_script: str, returns_cost: bool = False) -> float or None:
        """
        :param ddl_script: a DDL script
        :param returns_cost: if True, returns the cost incurred by the query
        :return: the cost incurred by the query if requested; else None
        """
        query_job = self.bq_client.query(query=ddl_script, location='EU')
        query_job.result()
        if returns_cost:
            return self._get_cost_from_processed_bytes(total_bytes_processed=query_job.total_bytes_processed)
    ...
    def query_to_dataframe(self, query: str):
        ...
        :return: pandas.DataFrame and the cost of query
        """
        if self.check_query_validity(query=query):
            query_job = self.bq_client.query(query=query)
            return query_job.to_dataframe(), self._get_cost_from_processed_bytes(
    ...
    def exists(self, table_name: str, project_name: str = None, dataset_name: str = None):
        """
        Check the existence of table_name at the default project_name.dataset_name
        :param table_name: name of the table
        :param project_name: project to look in.  Default is None; in which case the attributed project_name is used
        :param dataset_name : dataset to look in. Default is None; in which case the attributed dataset is used
        :return: True if table_name exists in the default project_name.dataset_name else False
        """
        dataset_name = dataset_name if dataset_name is not None else self.dataset_name
    ...
    def query_cost(self, query: str) -> float:
        """
        :param query:
        ...
        try:
            ...
            # 20 MB data processed per table referenced by the query,
            # and with a minimum 20 MB data processed per query
            # """"
            processed_bytes = min(query_job.total_bytes_processed,
                                  20*(1024**2)
                                  )
    ...
    def check_query_validity(self, query: str):
        ...
        # Start the query, passing in the extra configuration_file.
        try:
    ...
    @staticmethod
    def _get_cost_from_processed_bytes(total_bytes_processed) -> float:
        """
        :param total_bytes_processed: number of bytes processed by a query
        ...
        # and with a minimum 20 MB data processed per query
        # """"
        processed_bytes = max(total_bytes_processed,
                              20 * (1024 ** 2)
                              )
        return 5 * processed_bytes / (1024 ** 4)
