...
IMAGE_URI = "{location}-docker.pkg.dev/{project_name}/{repo_name}/{image_name}:{version}"
...
if __name__ == '__main__':
    ...
    if args.env == 'cloud':  # args.task == 'train':
        ...
        container_uri = IMAGE_URI.format(location=vertex_ai_location,
                                         project_name=project_name,
                                         repo_name=config['vertex_ai'].get('repository'),
                                         image_name=config['vertex_ai'].get('image'),
        ...
        if args.task == 'train':
            ...
            model = vertex_jobs.get_custom_container_job(
                dataset=dataset,
                project=project_name,
                location=vertex_ai_location,
                staging_bucket=os.path.join(bucket_name, titanic_dir, 'vertex_exp_custom_image'),
            ...
            endpoint_name = f'cs_img_endpoint_{model_display_name_prefix}_version_{args.endpoint_version}'
